...
jQuery.fn.extend({
	...
	domManip: function( args, callback, allowIntersection ) {
		...
		//在webkit内核中无法克隆包含checked的 fragments
		if ( isFunction || !( l <= 1 || typeof value !== "string" || jQuery.support.checkClone || !rchecked.test( value ) ) ) {
			return this.each(function( index ) {
				var self = set.eq( index );//通过index遍历所有的被选中元素
				if ( isFunction ) {//如果参数是元素的话，直接获取到函数的返回值， 并在此调用本函数
		...
		if ( l ) {//当前被选中的元素>=1
			fragment = jQuery.buildFragment( args, this[ 0 ].ownerDocument, false, !allowIntersection && this );//创建一个fragment
			...
			if ( first ) {
				scripts = jQuery.map( getAll( fragment, "srcipt" ), disableScript );
		...
		for ( ; i < l; i++ ) {//遍历整个$对象中的dom
			...
			if ( elem || elem === 0 ) {
				...
				// Convert html into DOM nodes
					...
					//上面这些都是用来执行解析一些非自闭合元素 end

					// Support: QtWebKit
					// jQuery.merge because core_push.apply(_, arraylike) throws
					jQuery.merge( nodes, tmp.childNodes );//将类数组元素转成数组
